...
label_num = {'0':45,"1":78,'2':124,'3':117,'4':79, '5': 40, '6':125,'7':43,'8':110, '9':92, '10':167, '11':160, '12':42, '13':100, '14':143}
    ...
    with torch.no_grad():
        ...
        if cat == 'object':
            ...
            for i in range(456, 600):
                ...
                for j in range(np.shape(reconstructions)[0]):
                    temp = np.array(reconstructions[j][i]).transpose([1, 2, 0])
                    temp_map = np.array(reconstructions[j][i]-data_native[i]).transpose([1, 2, 0])
                    if reverse_flag[normal_classes] == 1:
                        plt.imshow(1 - temp)
                    else:
                        plt.imshow(temp)

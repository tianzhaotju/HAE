...
def main():
    ...
    with torch.no_grad():
            ...
            for i in range(10):
              ...
              for i in range(6):
                ...
                reconstructions.append(
                    np.load('../.././log/' + cat + '/' + normal_classes + '/Data_Reconstruction_' + str(i) + '.npy'))
        ...
        if ae_loss_type == 'ssim':
            ...
            for i in range(np.shape(reconstructions_train)[0]):
                if i == 0:
            ...
            for i in range(np.shape(reconstructions)[0]):
                if i == 0:
        ...
        if cat == 'object':
            ...
            for i in range(np.shape(features_s)[0]):
                ...
                np.save('../.././log/' + cat + '/' + normal_classes + '/Single_Scores_' + str(i) + '.npy', scores)
            ...
            for i in range(np.shape(features_cal)[0]):
                ...
                labels[0:good_num[normal_classes]] = 0
                scores = np.array(scores)
                np.save('../.././log/' + cat + '/' + normal_classes + '/Cal_Scores_' + str(i) + '.npy', scores)
            ...
            np.save('../.././log/' + cat + '/' + normal_classes + '/KNN_score_s' + '.npy', pre_s)
            np.save('../.././log/' + cat + '/' + normal_classes + '/KNN_score_cal' + '.npy', pre_cal)
            np.save('../.././log/' + cat + '/' + normal_classes + '/Labels' + '.npy', labels)
            ...
            for i in range(22, 380):
                ...
                temp = np.squeeze(np.array(data_native[i]), axis=0)
                if reverse_flag[normal_classes]==1:
                    plt.imshow(1-temp)
                else:
                    plt.imshow(temp)
                ...
                for j in range(len(reconstructions)-1):
                    temp = np.squeeze(np.array(reconstructions[j][i]), axis=0)
                    if reverse_flag[normal_classes] == 1:
                        plt.imshow(1 - temp)
                    else:
                        plt.imshow(temp)

    ...
    with torch.no_grad():
        ...
        for i in range(15):
            ...
            for j in range(10):
                ...
                scores_train = np.sum((reconstruction_train - image_train)**2, axis=(1,2,3))
                ...
                labelNormal = Labels*np.sum((reconstruction_test - image_test)**2, axis=(1,2,3))
        ...
        image_samples = np.concatenate([image_samples,detected_KNN_collection], 0 )
        # image_samples = np.transpose(image_samples, [0,2,3,1])

        image_samples = torch.from_numpy(image_samples)

        plot_images_grid(image_samples, export_img='./pictures/Mnist_normal_abnormal_outlier.eps', nrow=10, padding=2)

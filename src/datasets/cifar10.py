...
class CIFAR10_Dataset(TorchvisionDataset):
    ...
    def __init__(self, root: str, normal_class=5):
        ...
        for i in range(len(self.normal_classes)):
            self.outlier_classes.remove(self.normal_classes[i])
        ...
        #                                 transforms.Normalize([min_max[self.normal_classes][0]] * 3,
        #                                                      [min_max[self.normal_classes][1] - min_max[self.normal_classes][0]] * 3)])
        #
        # target_transform = transforms.Lambda(lambda x: int(x in self.outlier_classes))


        # 1. Compose means combine multiple transforms into the same transforms
        #(1) To tensor, tranform array into pytorch tensor
        #(2)

        min_value = 0
        max_value = 0

        for e in self.normal_classes:
